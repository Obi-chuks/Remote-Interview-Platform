...
import ReacordingCard from "@/components/RecordingCard";
import { ScrollArea } from "@/components/ui/scroll-area";
import useGetCalls from "@/hooks/useGetCalla"
...
function RecordingsPage() {
  ...
  useEffect(() => {
    ...
    setRecordings(allRecordings);
        ...
        console.log("Error fetching recordingsS:", error);
       ...
       <ScrollArea className="h-[calc(100vh-12rem)] mt-3">
         {recordings.length > 0 ? (
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-6">
             {recordings.map((r) => (
               <ReacordingCard key={r.end_time} recording={r} />
             ))}
           </div>
         ) : (
